...
class StateNodeHash(AbstractStateNode):
    ...
    def rollout(self, obs, curr_depth: int):
        ...
        while not done and curr_depth + starting_depth < param_mcts.max_depth:
            ...
            if param_mcts.dynamic_coordination_graph:
                obs, rew, done = param_mcts.env.step(param_mcts.env.mdp, obs, sampled_action)
            else:
                obs, rew, done = param_mcts.env.step(obs, sampled_action)
...
class ActionNodeHash(AbstractActionNode):
    ...
    def build_tree_action(self, state, curr_depth) -> float:
        ...
        """
        if param_mcts.dynamic_coordination_graph:
            observation, instant_reward, terminal = param_mcts.env.step(param_mcts.env.mdp, state, self.data)
        else:
            observation, instant_reward, terminal = param_mcts.env.step(state, self.data)
