...
# Set this variable to false to use Variable Elimination
USE_MAX_PLUS = True
...
# Static version with bootstrapped constraints (we can prove the safety on global action)
def max_plus_spibb(node: AbstractStateNode, node_exploration=False):
    ...
    for aidx in range(node.param.number_of_agents):
        ...
        if str(s) not in param_spibb.mask[aidx].keys():
            ...
            if not param_spibb.mask[aidx][str(s)][action[aidx]]:
                bootstrapped = True
    ...
    - Propagate the value of the non-bootstrapped actions.
    """
    ...
    """
    for i in range(node.param.number_of_agents):
        Q_a = node.Q[i].copy()
        if node_exploration:
        ...
        s = [node.data[i]]
        for neigh in list(param_mcts.env.coordgraph.neighbors(i)):
            s.append(node.data[neigh])
...
# Static version with bootstrapped constraints (we can prove the safety on global action)
def var_el_spibb(node: AbstractStateNode, node_exploration=False):
    action = np.full(node.param.number_of_agents, -1)
    """ PART 1: Select bootstrapped action or not
    - Each agent selects independently an action. If it exists an agent that selects with the baseline, then all the 
    agents are constrained to select with the baseline
    ...
    "##################################################################################################################"
    """ PART 2: Var_El and action selection
    """
     # Keep only non-boot actions for each agent
    actions_per_agent = dict()
    for a in range(node.param.number_of_agents):
        s = [node.data[a]]
        for neigh in list(param_mcts.env.coordgraph.neighbors(a)):
            s.append(node.data[neigh])
        temp = [param_mcts.env.action_to_int(act) for act in param_mcts.env.agent_actions(param_mcts.env.mdp, a, node.data)]
        if str(s) in param_spibb.mask[a].keys():
            actions_per_agent[a] = [j for j in temp if param_spibb.mask[a][str(s)][j]]
        else:
            actions_per_agent[a] = []
...
class FV_MCTS_SPIBB_static(AbstractMcts):
    ...
    def fit(self) -> int | np.ndarray:
        ...
        if USE_MAX_PLUS:
            ...
            a_star = var_el_spibb(self.root, node_exploration=False)
...
class StateNodeHash(AbstractStateNode):
    ...
    def build_tree_state(self, curr_depth):
        ...
        if USE_MAX_PLUS:
            ...
            action = var_el_spibb(self, node_exploration=True)
        # end = time.time()
    ...
    def initialize_graph_statistics(self, env):
        ...
        if not param_mcts.dynamic_coordination_graph:
            ...
            self.coordgraph = env.coordination_graph(env.mdp, self.data[:-1])
    ...
    def rollout(self, obs, curr_depth: int):
        ...
        while not done and curr_depth + starting_depth < param_mcts.max_depth:
            ...
            for aidx in range(param_mcts.number_of_agents):
                ...
                if str(s) not in param_spibb.mask[aidx].keys():
                    sampled_action[aidx] = param_spibb.pi_b.get_single_act_from_baseline(s[0], aidx)
                    continue
